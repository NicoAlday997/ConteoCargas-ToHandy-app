...
| GET | `/admin/permisos-carga` | Supervisor | Permisos vigentes (sin usar y sin vencer), del que vence antes al que vence después: `{ permisos: [{ id, rutaId, rutaNombre, otorgadoPorId, otorgadoPorNombre, motivo, fechaOtorgado, fechaExpiracion }] }`. |
