...
| GET | `/eventos-carga/dias-recargables` | Vendedor | Días en que el vendedor puede iniciar una recarga: las cargas `INICIAL` en estado `ENVIADA` de su ruta asignada con `fechaOperativa` de hoy (día de negocio en America/Mexico_City) en adelante. Respuesta: `{ dias: [{ fechaOperativa, eventoInicialId }] }`, en orden ascendente por fecha; `fechaOperativa` es el inicio de ese día en hora de México (ISO 8601), igual que en el evento. Sin asignación vigente o sin salidas enviadas responde `{ dias: [] }`. La app ofrece solo estos días: con cero no deja iniciar la recarga, con uno pide solo confirmación, con varios muestra únicamente esos. |
